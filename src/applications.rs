...
use twilight_model::application::{command::CommandType, interaction::{Interaction, InteractionData, InteractionType}};
...
pub async fn set_application_command(state: &StateRef) -> anyhow::Result<()> {
    ...
    let join = CommandBuilder::new("join", "読み上げ開始", CommandType::ChatInput)
        .build();
...
pub async fn handle_interaction(state: &StateRef, interaction: Interaction) -> anyhow::Result<()> {
    ...
    match interaction.kind {
        InteractionType::ApplicationCommand => {
            let command = if let Some(InteractionData::ApplicationCommand(command)) = interaction.data {
                command
            } else {
                return Ok(());
            };
